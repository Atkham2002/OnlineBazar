...
public class ExceptionsHandler {
    ...
    public ResponseDto<List<ValidatorDto>> validatorExceptions(MethodArgumentNotValidException exception){
        List<ValidatorDto> errors = (List<ValidatorDto>) exception.getBindingResult().getFieldErrors().stream().map(fieldError -> new ValidatorDto(fieldError.getField(), fieldError.getDefaultMessage()));
